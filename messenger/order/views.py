...
def order_create(request):
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            for c in cart:
                OrderItem.objects.create(order_id=order.id,user_id=request.user.id,product_id=c.product_id,quantity=c.quantity)
...
def factor(request,order_id):
    ...
    if order.paid==True:
        ...
        email=order.email
        msg='Your order has been successfully registered. You can see order factor in your profile.'
        body=subject+'\n'+email+'\n'+msg
